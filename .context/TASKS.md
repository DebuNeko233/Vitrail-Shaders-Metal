...
- [ ] P5 - Remove the Vulkan path, last, in reviewable batches: `HostReport` and the status pages together first, then the Vulkan-only mixins, then the contract test and its `build.yml` line in one commit, then the prose. The seam stays: one backend, not no boundary.
