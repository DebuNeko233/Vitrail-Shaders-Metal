...
- [ ] Both PRs are still Draft/open/unmerged, and this item closes when they are not. The owner has decided to land them, companion `metallum#1` first and `Vitrail-Shaders-Metal#1` second, so the Vitrail merge never lands a Metal path on `dev` with no backend API behind it. That decision is recorded; the merges are not, and PHASE 17's closure promoted no pack, so nothing about the compatibility matrix changes when they happen.
