...
- [ ] Read the next hardware run's fatal marker and not only its frame count.
  - `tests/phase17_collect_session.py` treats `Vitrail stopped drawing this pack after an error` as a high-confidence fatal, and `tests/phase17_compatibility.py` returns `Broken` on it. A session whose shadow stage dies without that line would be filed as a fallback rather than a failure, which is the one reading the rerun that closes the list-overflow regression must not be given.
  - The former Sodium camera-list overflow stops the shadow stage, so the reruns in `docs/metallum-port.md`'s acceptance list are checked for that line explicitly.
...
- [x] Pin code-bearing heads rather than branch tips in every status surface.
  ...
  - The same correction is owed to both PR bodies, which still named `16b051c1` and `82a0c75e` as current heads while the branches had moved. Update them in the same pass, and keep them in the code-bearing-head form so a behaviour-neutral commit does not require another edit.
