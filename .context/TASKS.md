...
- [ ] P0 - Still owed: the second (light) pack run for the comparison, and a GPU trace of one frame kept beside the numbers. Until those exist, P1 is judged against a single session. A warm number needs the same build twice, or two loads inside one session: both disk stores name their directory after `Vitrail.cacheEdition()`, which adds the commit for a development build, so a rebuilt jar reads an empty edition and comparing across a rebuild compares two cold starts.
- [ ] P1 - Attachment lifetime and precise load/store. **The target is 2066 MiB of stores per frame, not the 313 this line used to carry.** That earlier figure came from the window recorded as the first baseline, and none of that window's 600 frames is a frame of a drawn pack: the session's only `first full frame` is eleven seconds after it closed, so those per-frame numbers describe warming frames with the level skipped. The first window that does contain a drawn pack reads **1689 MiB loaded and 2066 MiB stored per frame with 34 encoder boundaries, 33 of them a pass configuration change**, over 600 frames of Photon at 35 frames a second - eight and a half times the load traffic, which is the difference between a frame with no level and a frame with one. **First on the list now that the trace says the frame is GPU-bound and fragment-dominated**, so what this phase removes is frame time rather than traffic, and `MetalFrameProbe`'s `loadedMiB` and `storedMiB` measure the change directly without any per-pass attribution.
  - **Both halves are written, behind `-Dvitrail.elideTargetTraffic`, off by default.** A pass that writes every pixel of a target - no `discard`, no sampler naming a target it writes on the same half, and a draw over the whole screen - has a clear handed to the descriptor where the null stood, which is a tile fill instead of the target's bytes read back. A clear the frame already owes is never replaced. The run it needs: same scene with the property on and off, the image unchanged image for image, `loadedMiB` down, and `storedMiB`, encoder boundaries and bindings not regressing. If that holds, the default flips.
