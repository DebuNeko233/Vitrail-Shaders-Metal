...
- [x] Implement the generic source correction without Photon names, Photon center arithmetic, shader-option forcing or Metallum pack policy.
  ...
  - Sodium's exact camera `Viewport` / fog inputs and `updateChunksImmediately` choice are captured from `setupTerrain`. The light walk uses a sign-bit-flipped frame token and draws directly through Sodium 0.9.2's `drawChunkLayer`; a camera traversal then repairs the persistent region-list contents and the manager's original render-list/tree/task/flag state is restored.
