...
- [ ] P3 - Re-aimed at the module store rather than at pipeline state creation, which the baseline measured at 32 ms for the whole session. `making modules` (shaderc and SPIRV-Cross) is the largest item in every load recorded - 3524 ms over 328 modules with 124 built cold, and 1553 ms over 324 with 73 built on a reopen the translation store answered in full - while a load that served all 229 of its modules paid 39 ms for them, so a module the store answers is free and one it does not is 20 to 28 ms. The question to answer first is what those 73 to 124 built modules differ by in the key; the translation store's own cold miss, where one load served nothing while other moments in the same session served 48 and 63 programs, is the second question and no longer the first.
