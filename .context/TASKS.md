...
- [x] Implement the generic source correction without Photon names, Photon center arithmetic, shader-option forcing or Metallum pack policy.
  ...
  - Sodium's exact camera `Viewport` / fog inputs are captured from `setupTerrain`. The light walk uses a sign-bit-flipped frame token and draws directly through Sodium 0.9.2's `drawChunkLayer`; the saved camera `renderTree` selects the matching private list-builder to repair persistent region-list contents without calling `finalizeRenderLists` (and therefore without mutating camera timing control) a second time. The manager's original render-list/tree/task/flag state is restored afterwards.
