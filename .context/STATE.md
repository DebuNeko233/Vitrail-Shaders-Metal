...
- Current source moves the light walk, voxelising shadow draw and `shadowcomp` into the same frame. It captures Sodium's exact camera viewport, gives the light walk a shadow-only frame token, prepares/draws its lists, synchronously repopulates any camera region lists it overwrote, then restores the camera manager references/flags before Sodium prepares the main-world batches. No Photon formula, option name or Metallum rule is used. CI and fresh hardware validation are still required.
