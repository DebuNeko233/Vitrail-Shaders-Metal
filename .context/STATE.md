...
- The latest hardware evidence baseline is Vitrail `97dcf76d35ffbff9308ac293f73173b2f93dc2cd` paired with Metallum `82a0c75e53e28390472b3c26b569cdc2335d90b4` on Apple M5 Pro / macOS 27.0 / Metal. PHASE 2 and PHASE 5-16 have completed their recorded Apple-Silicon real-device acceptance. PHASE 17 — Real Shader Pack Compatibility — remains active.
...
- Photon view-centred LPV is source-fixed but not yet hardware-closed. The current source removes the one-frame shadow/compute split so a pack-defined view centre is shared by the identity writer and shadow compute. Keep it open until the Ahead-mode stationary-rotation rerun is stable on hardware; do not replace the fix with a Photon-specific macro, option override or Metallum image-copy rule.
...
- Re-run Photon with `Voxel Volume Center = Ahead` around the same stationary emissive source and rotate without translating. The Player-mode A/B has already isolated the view-centre mismatch; acceptance now requires the Ahead path to stop shifting/flickering with the same-frame source fix.
