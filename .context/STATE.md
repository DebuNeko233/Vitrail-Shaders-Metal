...
- Current source moves the light walk, voxelising shadow draw and `shadowcomp` into the same frame. It captures Sodium's exact camera viewport, gives the light walk a shadow-only frame token and draws those lists directly through the Minecraft 26.2 / Sodium 0.9.2 renderer. Camera-list restoration bypasses `finalizeRenderLists` so it does not advance Sodium's camera timing control a second time: the saved camera `renderTree` selects the same regular-tree or synchronous-fallback list builder, after which the manager references/flags are restored. No Photon formula, option name or Metallum rule is used. CI and fresh hardware validation are still required.
