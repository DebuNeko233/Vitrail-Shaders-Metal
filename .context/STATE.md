...
- Photon view-centred LPV remains an open Vitrail scheduling problem. The current whole-block storage-image reanchor repairs camera translation across the one-frame shadow/compute split, but does not repair a pack-defined voxel center that changes with view rotation. Do not hide this with a Photon-specific macro or a Metallum image-copy rule; the fix has to preserve the generic shader-pack scheduling boundary.
...
- A/B Photon with `Voxel Volume Center = Player` while stationary and rotating the view if practical. This is a diagnostic only: disappearance of the shift/flicker would isolate the missing view-center term; it must not become an automatic pack setting override.
- Design a generic correction for view-centred voxel identity timing across Vitrail's one-frame shadow/compute split, then re-run Photon around a stationary emissive source while rotating the view. Do not encode Photon's center formula or option names into production code.
