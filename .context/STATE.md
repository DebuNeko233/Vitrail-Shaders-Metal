...
- Current source moves the light walk, voxelising shadow draw and `shadowcomp` into the same frame. It captures Sodium's exact camera viewport plus its synchronous-update choice, gives the light walk a shadow-only frame token, draws those lists directly through the Minecraft 26.2 / Sodium 0.9.2 renderer, then repopulates any camera region lists it overwrote and restores the camera manager references/flags. No Photon formula, option name or Metallum rule is used. CI and fresh hardware validation are still required.
