...
本文件是 `Vitrail-Shaders-Metal` 的仓库级开发契约。下方迁移路线图作为后续实现、评审和验证的长期约束。

额外强制规则：

1. **目标版本固定为 Minecraft Java Edition 26.2。**
2. **Sodium 使用 Minecraft 26.2 可用的最新版本。** 在修改 Sodium 相关代码前，必须先核对当前 26.2 release、文档与对应源码，并固定经过验证的精确版本。
3. **每次引用、增加或修改外部 API 前，必须查询该目标版本的当前文档；文档不足时必须直接查询对应版本源码。** 不允许仅凭记忆猜测版本敏感 API。
4. **每次代码修改完成后，必须立即检查并同步相关文档。** 至少检查 `AGENTS.md`、README、架构/兼容性说明、已知限制以及对应开发记录是否需要更新。
5. **Vitrail / Metallum 边界不可破坏。** Shader Pack policy / semantics 属于 Vitrail；Metallum 只实现 backend GPU behavior。
6. **Minecraft 26.2 Graphics API 和 Sodium 集成属于版本敏感接口。** 每次实现前都必须以 26.2 文档/源码为准。
7. **禁止静默兼容性猜测。** 未支持或尚未验证的行为必须在代码、日志和文档中明确，并在架构允许时安全回退。
