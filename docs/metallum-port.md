...
### Metal capability providers

Two `@Pseudo` mixins bridge package-private Metallum classes without putting Metallum on Vitrail's common compile classpath, and both name their target by string for that reason: `MetalBackendMixin` publishes the independent-blend fact after `MetalBackend#createDevice` returns, and `MetalDeviceMixin` carries the five device-level capabilities - selective pipeline eviction, shader-storage-buffer allocation, writable storage-image allocation, ordinary shader-writable texture allocation, and compute pipeline/dispatch - through the compat bridges.

The encoder-level capabilities are supplied by an adapter rather than injected. `Backends.capabilities` answers the backend itself where it carries them, the flat bridge's adapter where Metallum's stable bridge answers `supports`, and `MetallumEncoderCapabilities` otherwise, and that adapter is what carries mipmap generation, storage-image clear/copy, compute encoding, attachment commands and the scale commands. The injection that used to put those methods onto a named Metallum encoder left with the class that moved, silently; `tests/test_backend_capability_reach.py` and `tests/test_shadow_mipmap_contract.py` refuse that shape again.

Metallum is a required runtime dependency of this engine and not an optional integration: the mod metadata declares its floor, and a session without it, with an API of another version, or without a Metal device is a startup failure naming which of the three it was rather than a session that quietly draws nothing.
...
**Those closures are `dev`'s and were taken before the other backend's implementation was deleted**, so they do not carry to the converted tree by themselves. What has been driven on that tree, through the framebuffer-capture recipe in [`.context/STATE.md`](../.context/STATE.md), is eight deterministic gates - `mrt-contract`, `wide-resources-contract`, `composite-history-contract`, `composite-flip-contract`, `compute-storage-contract`, `phase16-advanced-contract` (3D-volume read and hardware comparison sampler included), `shadow-mipmap-contract` on Metal 3 and `deferred-mipmap-contract` on Metal 4 - plus a real pack's own load (Complementary Reimagined r5.9.1, 62 of 62 warm-up pipelines and no error) and a device run of the start-up refusal for a Metallum API of another version, which came up on Metal, named the two versions in its own clause and drew no pack. That recipe also records the one scene control those gates need: the client's vignette option off, because the vanilla HUD otherwise multiplies the finished frame by its own texture and an outer quarter can read as a failure the pack did not produce. The rest of the fixture corpus, the two remaining start-up refusals and the resize/reload behaviours are `.context/TASKS.md`'s own section rather than anything this page can claim.
