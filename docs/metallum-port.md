...
At this status update the companion Metallum code-bearing head is `82a0c75e53e28390472b3c26b569cdc2335d90b4`. This page names code-bearing heads rather than branch tips, because commits after one that touch only documentation, CI, tests or `.gitignore` change no rendered behaviour, and a tip that moves for those reasons would make every statement here stale without saying anything new. The Metal foundation and baseline PHASE 5-16 acceptance already have Apple-Silicon real-device evidence; PHASE 17 real-pack compatibility remains active and may still expose generic defects that must be fixed in their owning subsystem.

## Vitrail backend-neutralization

The backend-neutralization baseline is implemented on branch `feat/backend-neutral-sodium-terrain-hook` and Draft PR #1, targeting `dev` as required by repository policy. Its code-bearing head is `484fdd2d4dc2fd80f7cb1254db4869cbcee1d0a2`, whose behaviour-neutral documentation successor `5ab260ab1c007cec53f22eb925954e93cb7dc951` is the head the latest device sessions and the latest CI run both name. The two defects that used to be outstanding at the head of this page are both closed: the `60ff5610cf3ce60f05fcd0bbe1566b32fa00adb4` Sodium camera-list restoration overflow is repaired at `faed8edadf705440419bdfb7049f10127c3e607b` and confirmed on hardware across the sessions at that repair's successor `16b051c1c767c50269cbd67ff48220f82edfe8f3`, and the shadow matrix publish defect that `cd52b13f` introduced is repaired at `484fdd2d`, with the tester confirming on hardware that the cutout-vegetation ghosting is gone. The sky-ownership path that the earlier `0caa74ba6f91f97584cfad0d8d9172a30181986a` attempt broke was repaired and carries structural Apple-Silicon evidence at `ac33fed3fa3323784c24bc6c96db82d828c51393`.
...
Both repositories are still Draft, open and unmerged. The owner has decided to land them, companion first and this branch second, so that the Vitrail merge never finds a `dev` whose Metal path has no backend API behind it; the merge itself is the only step left, and until it happens the code-bearing heads above are what is on the branches rather than on `dev`. The Metal shader-pack route remains developer-validation-only.
...
3. **Not done.** No PHASE 17 screenshot/reference material was collected. The tester's Bliss scene-seed check is a useful regression observation and is not the reviewed evidence the policy requires, and the End sky branch, the other `covers=true` branch, has still never been exercised;
