...
- **The cost is paid at load, not per frame.** Some of it lands at selection and the rest in the
  ...
  world: for those seconds the screen is Vitrail's loading page, drawn in the same shape the game
  uses while it waits for terrain, with the Vitrail mark over "Compiling shaders..." and a bar under
  it filling with the programs compiled so far. The leftover families then compile in the background
  while you play, and there the mark drops back to the top-left corner with those words beside it,
  the count still riding along, and a first draw that outruns that work compiles on the spot. When
  everything is in, the mark stands still, says so, and the corner fades back to the game. The same
