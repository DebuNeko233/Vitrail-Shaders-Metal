...
/** Developer-only pixel verifier shared by the PHASE 7 hand and hand_water fixtures. */
public final class VerifyHandScreenshot {
    private VerifyHandScreenshot() {
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length != 1) {
            System.err.println("Usage: java tests/VerifyHandScreenshot.java <minecraft-screenshot.png>");
            System.exit(2);
        }
        ...
        System.out.println("Hand screenshot swatches: GREEN=" + counts.green()
                + " MAGENTA=" + counts.magenta() + " greenMinimum=" + counts.greenMinimum());
        if (!counts.passes()) {
            throw new AssertionError("Expected a visible green first-person hand region and no substantial magenta ABI failure region");
        }
        System.out.println("Hand screenshot routing/vertex-ABI colour check: PASS");
    }
    ...
    private static void selfTest() {
        ...
        if (absent.passes()) throw new AssertionError("Verifier missing-hand self-test unexpectedly passed: " + absent);

        System.out.println("Hand screenshot verifier self-test: PASS " + good);
    }
