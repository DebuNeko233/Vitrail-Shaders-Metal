...
/** Pixel verifier for the PHASE 14 >16-sampler wide-resource checkpoint. */
public final class VerifyWideResourcesScreenshot {
    private VerifyWideResourcesScreenshot() {}

    public static void main(String[] args) throws Exception {
        if (args.length == 1 && "--self-test".equals(args[0])) { selfTest(); return; }
        if (args.length != 1) {
            System.err.println("Usage: java tests/VerifyWideResourcesScreenshot.java <minecraft-screenshot.png>");
            System.exit(2);
        }
        BufferedImage image = ImageIO.read(new File(args[0]));
        if (image == null) throw new IllegalArgumentException("Not a readable image: " + args[0]);
        Counts c = count(image);
        System.out.println("wide-resource screenshot swatches: GREEN=" + c.green + " MAGENTA=" + c.magenta + " OTHER=" + c.other);
        if (!pass(c)) throw new AssertionError("PHASE 14 wide-resource screenshot contract failed");
        System.out.println("PHASE 14 wide-resource screenshot check: PASS");
    }
    ...
    private static void selfTest() {
        ...
        System.out.println("Wide-resource screenshot verifier self-test: PASS");
    }
