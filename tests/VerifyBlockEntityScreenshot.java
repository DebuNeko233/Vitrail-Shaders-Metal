...
/** Developer-only pixel verifier for the PHASE 7 block-entity fixture. */
public final class VerifyBlockEntityScreenshot {
    private VerifyBlockEntityScreenshot() {
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length != 1) {
            System.err.println("Usage: java tests/VerifyBlockEntityScreenshot.java <minecraft-screenshot.png>");
            System.exit(2);
        }
        ...
        System.out.println("Block-entity screenshot swatches: RED=" + counts.red()
                + " GREEN=" + counts.green() + " MAGENTA=" + counts.magenta()
                + " greenMinimum=" + counts.greenMinimum());
        if (!counts.passes()) {
            throw new AssertionError("Expected red terrain, a visible green block entity, and no substantial magenta ABI failure region");
        }
        System.out.println("Block-entity screenshot routing/vertex-ABI colour check: PASS");
    }
    ...
    private static Counts count(BufferedImage image) {
        ...
        return new Counts(red, green, magenta, 64);
    }
    ...
    private static void selfTest() {
        ...
        System.out.println("Block-entity screenshot verifier self-test: PASS " + good);
    }

    private static void fill(BufferedImage image, int x0, int y0, int x1, int y1, int argb) {
        for (int y = y0; y < y1; y++) for (int x = x0; x < x1; x++) image.setRGB(x, y, argb);
    }
